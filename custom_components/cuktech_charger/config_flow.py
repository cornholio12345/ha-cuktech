"""Einrichtungsdialog für CUKTECH Charger."""
...
STEP_USER_DATA_SCHEMA = vol.Schema({
    vol.Optional(CONF_NAME, default="CUKTECH 10 Ultra"): str,
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, str]:
    ...
    try:
        async with session.get(f"{server_url}/api/status", timeout=10) as resp:
            if resp.status != 200:
                raise ValueError(f"Server antwortete mit HTTP {resp.status}")
            await resp.read()
    except ValueError:
        raise
    except Exception as err:
        raise ValueError(f"Verbindung zum Server fehlgeschlagen: {err}") from err
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                ...
                name = user_input.get(CONF_NAME, "CUKTECH Charger")
    ...
    async def async_step_reauth_confirm(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                ...
                _LOGGER.exception("Unerwarteter Fehler bei der erneuten Verbindung")
                errors["base"] = "unknown"
            else:
